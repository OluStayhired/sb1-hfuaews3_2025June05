...
interface ScheduleDraftPostProps {
  ...
  initialContent?: string; // New prop
  //onScheduleSuccess: (postData: any) => void;
}
...
export function ScheduleDraftPost({ 
  isOpen, 
  onClose, 
  //selectedDate, 
  selectedTime, 
  onSchedule, 
  onScheduleError, 
  initialContent }: ScheduleDraftPostProps) {
 ...
 // New state for datepicker visibility and ref
...
useEffect(() => {
  if (isOpen) { // Only update if modal is open
    setPostContent(initialContent || '');
  }
}, [isOpen, initialContent]); // Depend on isOpen and initialContent
  ...
  const validateAndSetDate = (date: Date, setDateError: React.Dispatch<React.SetStateAction<string | null>>) => {
    ...
    // If date is valid, update the state
    //setSelectedDate(date); // Assuming selectedDate is a state in the parent component
    return true;
  };
...
const handleTimeChange = (e: React.ChangeEvent<HTMLInputElement>) => {
 ...
 // New handleDateChange for the datepicker
  ...
  const newDate = new Date(e.target.value);
   if (validateAndSetDate(newDate, setDateError)) { // Ensure date is valid before setting
...
const handleGenerateContent = async () => {
  ...
  const handleUpdateToStartDate = () => {
    if (campaignStartDate) {
      // Assuming selectedDate is a prop, you'd call a prop function to update it
      // For this example, we'll just close the date picker.
      setIsDatePickerOpen(false);
      setCampaignDateMismatch(false);
    }
  };  
...
//------------------ End Image Functions ----------------------------// 
  ...
  try {
    ...
    // This calls onModalScheduleSuccess in the parent
    onSchedule(newPost); 
    ...
    setIsSaving(false); // Stop loading indicator on the button
    onClose(); // Close the modal

  } catch (err) {
    console.error('Error saving scheduled post:', err);
    onScheduleError(newPost); // Notify parent of error
    setIsSaving(false); // Ensure loading is off on error
  }
};
...
<div className="space-y-6">
      ...
      <div>
        ...
        <div className="relative">
          <button
            ...
            className={`flex w-full items-center justify-between px-4 py-2 text-sm cursor-pointer hover:border-blue-500 hover:bg-blue-50
 ...
                    validateAndSetDate(today, setDateError);
                    if (validateAndSetDate(today, setDateError)) {
                      setSelectedDate(today);
                      // Assuming selectedDate is a prop, you'd call a prop function to update it
                      // For this example, we'll just close the date picker.
                      setIsDatePickerOpen(false);
 ...
                    validateAndSetDate(tomorrow, setDateError);
                    if (validateAndSetDate(tomorrow, setDateError)) {
                      setSelectedDate(tomorrow);
                      // Assuming selectedDate is a prop, you'd call a prop function to update it
                      // For this example, we'll just close the date picker.
                      setIsDatePickerOpen(false);
...
{/* Error message */}
      <div className={`flex items-center space-x-2 ${
        timeError || dateError ? 'bg-red-50 border border-red-200' : 'hidden'
      } rounded-md p-2`}>
        {(timeError || dateError) && <AlertCircle className="text-red-300 w-5 h-5"/>}
        {timeError && (
  ...
  {/*---------------------- End NEW: Add Image Button ----------------- */}
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
             ...
             <p className="text-sm text-gray-600">{initialContent}</p>
          </div>
        </div>
       ); 
    ...
    <TooltipExtended text={getNextButtonTooltip()} show={!canProceedToSchedule()}>
        ...
        <TooltipExtended text={getNextButtonTooltip()} show={!postContent.trim()}>
          <button
            ...
            className={`px-6 py-2 ${
              timeError !== null || dateError !== null
                ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                : isSuccess // <--- Use isSuccess here for button color/text
                  ? 'bg-green-500 text-white' // Green for success feedback
          ...
          >
            ...
            ) : isSuccess ? ( // <--- Use isSuccess here for button text
              <>
